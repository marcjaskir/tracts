...
for tract in ${data_root}/qsirecon/${sub}/ses-V1/dwi/*.tck; do
    ...
    if [ ! -f ${outputs_dir_mgz}/${tract_fname}.mgz ]; then
        tckmap ${tract} -template ${data_root}/qsiprep/${sub}/ses-V1/dwi/${sub}_ses-V1_space-T1w_dwiref.nii.gz ${outputs_dir_mgz}/${sub}.${tract_label}.mgz
    fi

    # Convert voxelized tracts to NIFTIs
    mri_convert --in_type mgz \
        --out_type nii \
        ${outputs_dir_mgz}/${sub}.${tract_label}.mgz \
        ${outputs_dir_nifti_LPS}/${sub}.${tract_label}.LPS.nii.gz
    ...
    mri_convert --in_type nii \
                ...
                --out_orientation LAS+ \
                ${outputs_dir_nifti_LPS}/${sub}.${tract_label}.LPS.nii.gz \
                ${outputs_dir_nifti_LAS}/${sub}.${tract_label}.LAS.nii.gz
